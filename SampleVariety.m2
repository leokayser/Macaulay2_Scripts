...
-- param:    a (rational) dominant map from A^n to (the affine cone over) X
-- paramdim: The dimension of the domain A^n of param
...
grassSV (ZZ,ZZ) := o -> (k,n) -> (
    ...
    pluckerEmb := mat -> (
        A := id_(o.kk^k) | reshape(o.kk^k, o.kk^(n-k), mat);
        return matrix{apply(subsets(n,k), s -> det submatrix(A, s))};
    );
    params := new HashTable from {param => pluckerEmb, paramdim => k*(n-k), codim => binomial(n,k)-1-k*(n-k)};
    return new SampleVariety from merge(I, params, );
)

toricSV = method(Options => {kk => ZZ/101})
toricSV (Matrix) := o -> A -> (
    n := numColumns A;
    N := numRows A;
    ...
    monMap := mat -> matrix{
        {1} | for i to numRows(A)-1 list product(flatten entries mat,flatten entries A^{i}, (x,a)->x^a)
    };
    params := new HashTable from {param => monMap, paramdim => n, codim => N-n};
...
hilbFunTable = args -> (
    ...
    return transpose reverse for t to tmax list {tmax-t} | apply(Ms, M -> hilbertFunction(tmax-t, M));
)
...
disp = (sv,tmax) -> (
    ...
    return netList apply({{""},{" S "},{" I "},{" L "},{"I+L"},{"I_Z"}}, hilbFunTable(ring sv, module sv, module L,module J, module IZ, tmax), (a,b)->a|b);
)